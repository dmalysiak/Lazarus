...
#ifndef SHAREDMUTEXRW_H_
#define SHAREDMUTEXRW_H_
...
class SharedRWMutex: public Lazarus::BaseObject {
public:
	SharedRWMutex(SharedMemory* sm);
	virtual ~SharedRWMutex();
	...
	/**
	 * locks the mutex for read operations. 0 for success, -1 otherwise.
	 */
	int lockMutexR();

	/**
	 * locks the mutex for write operations. 0 for success, -1 otherwise.
	 */
	int lockMutexW();
...
private:
	...
	pthread_rwlock_t* mp_mutex;
	SharedMemory* mp_sm;
};

} /* namespace Lazarus */

#endif /* SHAREDMUTEXRW_H_ */
