/*
 * CallbackServerComFacility.cpp
 *
 *  Created on: 17.06.2013
 *      Author: darius
 */

#include "CallbackServerComFacility.h"
#include "../../Toolbox.h"

namespace Lazarus {

CallbackServerComFacility::CallbackServerComFacility() {

	mp_active_nodes = new Lazarus::ThreadSafeUnorderedMap<unsigned int,CallbackNode*>();
...
void CallbackServerComFacility::addNode(CallbackNode* node)
{
	//insert the node into the list
	mp_active_nodes->insertElement(node->m_node_id,node);
}

void CallbackServerComFacility::removeNode(unsigned int node_id)
{
	//remove the element
...
CallbackNode* CallbackServerComFacility::getNode(unsigned int node_id)
{
...
int CallbackServerComFacility::sendRequestToNode(unsigned int node_id, Lazarus::Buffer* request, SynchronizationCallback* callback)
{
	int result = 0;
...
int CallbackServerComFacility::sendRequestToMultipleNodes(Lazarus::FastKTuple<unsigned int>& node_ids, Lazarus::Buffer* request,
...
int CallbackServerComFacility::sendRequestToNodeNL(unsigned int node_id, Lazarus::Buffer* request, SynchronizationCallback* callback)
{
	int result = 0;
...
int CallbackServerComFacility::sendRequestToNodeAsync(unsigned int node_id, Lazarus::Buffer* request)
{
	int result = 0;
...
unsigned int CallbackServerComFacility::getNodeCount()
{
	return mp_active_nodes->getMap()->size();
}

Lazarus::FastKTuple<unsigned int>* CallbackServerComFacility::getNodeIDs()
{
	...
	for( std::unordered_map<unsigned int,CallbackNode*>::iterator it = mp_active_nodes->getMap()->begin();
...
///*
//unsigned int CallbackServerComFacility::acquire_free_node_id(unsigned int desired_id)
//{
//	/**
...
bool CallbackServerComFacility::check_id_usage(unsigned int id)
{
